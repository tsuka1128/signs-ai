...
export function usePlanFeatures() {
  ...
  const canUse = useCallback((feature: FeatureKey): boolean => {
    ...
    switch (feature) {
      ...
      case 'labor_analytics':
        ...
        return (plan?.enable_labor_analytics || company?.addon_labor_analytics) ?? false;
      case 'pdf_export':
  ...
  }, [company, plan, loading]);

  // 数量制限の取得
  const limits = useMemo(() => ({
    maxDepartments: plan?.max_departments ?? 3,
    maxKpis: plan?.max_kpis ?? 5,
    maxMembers: plan?.max_headcount ?? 20,
    manualAiRuns: plan?.manual_ai_runs_per_month ?? 1,
  }), [plan]);
