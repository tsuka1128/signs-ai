...
出力はJSON形式のみで返してください（Markdownコードブロック不要）。`;
        ...
        // ユーザープロンプト
        const prompt = `以下は今月の従業員アンケートのデータ（匿名、${respondentCount}名分）です。
...
次のJSON形式で返してください：
{
  ...
  "positive_summary": "ポジティブな声の箇条書き（自由コメントの記述のみを根拠に、実際に寄せられた前向きな声やエンゲージメント要因を「・…」から始まる改行区切りの箇条書きで抽出してください。AIによる解釈、分析、数値データは一切含めず、できるだけ生の言い回しやニュアンスを残してください（ただし人名・固有名詞・個人を識別しうる呼称は必ず除去・一般化すること））",
  "negative_summary": "課題・改善要望・不満の声の箇条書き（自由コメントの記述のみを根拠に、実際に寄せられた課題や要望、懸念事項を「・…」から始まる改行区切りの箇条書きで抽出してください。AIによる解釈、分析、数値データは一切含めず、できるだけ生の言い回しやニュアンスを残してください（ただし人名・固有名詞・個人を識別しうる呼称は必ず除去・一般化すること）。該当する声がない場合のみ「・目立った声はありませんでした」と出力してください）",
  "deep_dive": "深掘り分析：背景と示唆（自由コメントと定量スコア（設問別、体温、全社平均比、前月比）をクロス分析し、なぜこのような結果になっているかの背景要因と、今後想定される組織リスク、および取るべき方向性を論理的かつ具体的に3〜5文で論述してください）",
  "manager_hint": "マネージャーへの注目点（定量スコアとコメントから得られたインサイトに基づき、マネージャーが今月注目・実践すべき具体的なフィードバックやアクションのヒントを2〜3点、改行区切りの箇条書き形式で提示してください）"
}

注意：
- topics は5〜7件に増やして記述すること
- positive_summary および negative_summary には、スコアや比率などの数値データ（例: 「3.0を下回る」「全社平均比-0.5」など）を決して持ち込まないでください。これらは自由コメントの声をありのまま届けるセクションです。
- deep_dive および manager_hint には、定量データを十分に活用した分析結果を記述してください。
- 【最優先・匿名化】人名・個人名・AIエージェント名・特定の人物や役職者個人を指す固有名詞や呼称は、全セクションで必ず除去または一般化すること（例：「ドラッカーが目指す」→「目指していた状態」、「〇〇さんが言う」→主語を出さず「〜という声がある」）。生の言い回し・ニュアンスを残すのは「内容」についてであり、固有名詞や個人特定情報を残してよいという意味ではない。匿名化を最優先する。
- 特定の業務詳細など、回答者が誰か推測できる情報も含めないこと。
- コメントがない場合でも、定量スコアから示唆される組織状態に基づき、deep_dive および manager_hint は適切に記述すること（「なし」「—」等の空欄にしない）。`;
