...
次のJSON形式で返してください：
{
  ...
  "positive_summary": "ポジティブな声の箇条書き（自由コメントの記述のみを根拠に、実際に寄せられた前向きな声やエンゲージメント要因を「・…」から始まる改行区切りの箇条書きで抽出してください。AIによる解釈、分析、数値データは一切含めず、できるだけ生の言い回しやニュアンスを残してください）",
  "negative_summary": "課題・改善要望・不満の声の箇条書き（自由コメントの記述のみを根拠に、実際に寄せられた課題や要望、懸念事項を「・…」から始まる改行区切りの箇条書きで抽出してください。AIによる解釈、分析、数値データは一切含めず、できるだけ生の言い回しやニュアンスを残してください。該当する声がない場合のみ「・目立った声はありませんでした」と出力してください）",
...
- 個人が特定できる情報（個人名や特定の業務詳細）は全セクションにおいて一切含めないこと。
