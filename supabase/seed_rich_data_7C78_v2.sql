...
BEGIN
  ...
  IF comp_id IS NULL THEN
    RAISE EXCEPTION '対象の企業が見つかりませんでした。';
  END IF;
  ...
  FOR i IN 0..12 LOOP
    ...
    FOR kpi_rec IN SELECT id, target_default, name FROM kpi_definitions WHERE company_id = comp_id LOOP
      ...
      VALUES (kpi_rec.id, target_month_str::DATE, rand_val, rand_target);
      ...
      FOR axis_rec IN SELECT id FROM kpi_axes WHERE company_id = comp_id LOOP
        ...
        VALUES (kpi_rec.id, target_month_str::DATE, (rand_val / 3) * (0.5 + random()), rand_target / 3, axis_rec.id);
      END LOOP;
    END LOOP;
    ...
    FOR dept_rec IN SELECT id FROM departments WHERE company_id = comp_id LOOP
      FOR j IN 1..3 LOOP
        ...
        BEGIN
          ...
          VALUES (comp_id, dept_rec.id, target_month_str::DATE, (target_date + (random() * 27 || ' day')::INTERVAL), '組織環境の継続的な改善を希望します。')
